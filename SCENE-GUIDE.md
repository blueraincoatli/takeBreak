...
│   │   ├── face_landmarker.task        # 面部检测 ⚠️ WASM bug
│   │   └── hand_landmarker.task        # 手部检测
...
| `cat-paw` | Static | ✅ 已完成 |
| `squat` | Motion | ✅ 已完成 |
| `liquid-glass` | Static | 📋 目录已建，待实现 |
| `merit-plus-one` | Static | 📋 目录已建，待实现 |
| `boss-is-coming` | Static | 📋 目录已建，待实现 |
| `drink-water` | Motion | 📋 目录已建，待实现 |
